...
public class ConflictJRisk {
    ...
    public Set<String> getConflictLevel() {
        ...
        for (DepJarJRisk depJarJRisk : jarRisks) {
            ...
            try {
                PrintWriter printer = new PrintWriter(new BufferedWriter(new FileWriter(UserConf.getOutDir() + MavenUtil.i().getProjectCor() + "_time.txt", true)));
                printer.println(executionTime + "@@" + depJarJRisk.getUsedJar().toString() + "@@" + depJarJRisk.getConflictJar().toString());
                printer.close();
            } catch (IOException e) {
                e.printStackTrace();
            }
            MavenUtil.i().getLog().info("computation time@@" + executionTime + "@@" + depJarJRisk.getUsedJar().toString() + "@@" + depJarJRisk.getConflictJar().toString());
        }
...
//	 }
    public int getReachRiskMethods() {
        ...
        for (DepJarJRisk depJarJRisk : jarRisks) {
            Graph4path pathGraph = depJarJRisk.getGraph4mthdPath();
            pathBooks = new Dog(pathGraph).findRlt(pathGraph.getMiddleNodes(), Conf.DOG_DEP_FOR_DIS, Strategy.NOT_RESET_BOOK);
            if (depJarJRisk.getMethodBottomForPath(pathBooks).size() > 0) {
                return 2;
            } else {
                pathBooks = new Dog(pathGraph).findRlt(pathGraph.getHostNds(), Conf.DOG_DEP_FOR_DIS, Strategy.NOT_RESET_BOOK);
                if (depJarJRisk.getMethodBottomForPath(pathBooks).size() > 0) {
                    return 3;
                }
            }
